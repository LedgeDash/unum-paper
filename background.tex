...
\dhledit{standalone} service has significant drawbacks. Building performant,
scalable \dhledit{and fault-tolerant} multi-tenant systems is hard and
orchestrators introduce yet-another potential performance and scalability
bottleneck. \dhledit{Indeed, we find that, in practice, production systems
...
still incur engineering expenses as they require teams on-call. Indeed, we
find that provider-hosted orchestrators cost developers significantly and
dominate the total cost of running applicatoins (\S~\ref{sec:eval}).}

\dhledit{Lastly, provider-hosted orchestrators preclude users from making
...
the orchestrator might efficiently support the needs of many application, it
cannot meet all applications' needs, resulting in a compromise familiar from
operating systems~\cite{exokernel,spin}, networks~\cite{active-networks,sdn},
and storage systems~\cite{comet,splinter}. Indeed, we find that
provider-hosted orchestrators force applications to compromise performance by
using less-efficient but supported patterns (\S~\ref{sec:eval}).}
