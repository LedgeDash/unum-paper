...
\shadi{@David: please read and see what I have written is correct and does not change the message. If anything is colored, it means I am not sure about it.}
...
\emph{asynchronously} via storage triggers, (i.e., a function writes its result to a intermediate storage which in turn triggers the following function), and  \textit{ii.driver
	...
	\shadi{shouldn't the first figure (a) have a storage in the middle?} \shadi{from the figure and text it is not clear what is the diff between driver and coordinator. }
\shadi {let's just use trigger based throughout the paper}
...
Although both approaches are ``purely serverless'', i.e., they do not require adding
additional components to the serverless infrastructure and carry all the cost/scale/simplicity benefits of the serverless design, neither is well-suited for the  complexities of the currently emerging serverless applications which consists of \shadi{10s and 100s} of serverless functions \shadi{cite}. 
...
functions in the application complete, which risks hitting the commonly imposed timeouts of serverless platforms (e.g., \shadi{X sec in AWS[cite]}) and limits scalability. 
 \shadi{how about the fact that it is sync? doesn't that add delay, e.g., in a fan-out} 


Moreover, neither approach provides the commonly sought after strong  ``exactly once'' execution guarantee \shadi{citations on why this is required}.  Functions can crash mid-execution due to runtime
...
\dhl{Maybe explain why fan-in is not supported as a contrast to \name{}. But
	...
	consistent data stores and how we use them.} \shadi{Iagree that you should say why fan-in is not supported. does the current text handle that?}
