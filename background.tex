...
functions. Development quickly gets unwieldy as the number of functions in
the application increases. More importantly, it does not support all forms of
composition such as fan-in (aggregating results from multiple functions after
\textit{all} are complete).
...
lead to automatic retries~\cite{aws-lambda-retry, azure-functions-retry} and
potentially concurrent instances given the at-least-once execution currently
provided~\cite{aws-lambda-async-invoke, azure-functions-exec-guarantee}.
...
say why fan-in is not supported. does the current text handle that?}
