...
\dhl{This and the last para jumped into patterns without first defining them.
Why can't ad-hoc approach support fan-in? What is fan-in in serverless and why
do we need it? I think it'd be useful to describe the patterns at least
concisely to some extent in the bg, because it'll help the readers appreciate
the necessity and difficulty, and it will make the design section easier to
write since it won't be the first time readers see these patterns.}
...
While workflow orchestrators are able to address the needs of complex serverless
applications, introducing a new platform service has significant drawbacks.

Building performant, scalable multi-tenant systems is hard and orchestrators
introduce yet-another potential performance and scalability bottleneck.
Moreover, running such services can be expensive, as they require dedicated
resources in addition to those allocated to FaaS and storage systems.  Indeed,
we find (Section~\ref{sec:eval}) that, in practice, production systems limit
end-to-end performance and scalability, and cost developers significantly more
...
\squishlist
    ...
    \item \textbf{No new services: } The system should work within a basic,
    unmodified serverless environment and not require adding any new services.
