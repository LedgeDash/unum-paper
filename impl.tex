...
We implement a prototype \name{} runtime that supports \secondedits{AWS and
Google Cloud. We also implement a front-end compiler that transforms AWS Step
Function definitions to \name{} IR.} Currently our runtime only supports
...
store must be \secondedits{strongly consistent} with support for atomic
...
The error handler checks if the failed function should be retried (e.g.,
