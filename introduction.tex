...
\dhledit{Serverless computing offers a simple but powerful abstraction with
two essential components: stateless compute engines (Functions as a
Service, or FaaS) and scalable, multi-tenant data stores~\cite{berkeley}.
Developers build applications using stateless, event-driven ``functions''
which persist states in shared data stores. Both components offer users
important cost benefits in fine-grained pay-for-what-you-use billing and
freedom from any service management tasks.}
...
\dhledit{Cloud providers and application developers have introduced standalone
workflow orchestrators to support such applications
(\S\ref{sec:bg:orchestrator}). Many cloud providers offer serverless
...
orchestrators~\cite{temporal, gg-atc, excamera} and deploy in VMs or
containers alongside their functions.}

\dhledit{Although they are likely internally distributed, standalone
orchestrators work as \emph{logically centralized} controllers. Developers
provide a description of an execution graph---nodes in the graph represent
FaaS functions and edges represent invocations of a function with the output
of one or more functions---while the orchestrator drives the execution of this
graph by invoking functions, receiving function results and hosting
application states (outstanding invocations and intermediate results)
centrally.}
...
serverless providers and serverless users. As additional services that are
critical to application performance and correctness, standalone orchestrators
are difficult and expensive to build, host and scale. User-deployed
orchestrators risk under-utilization and break serverless' benefits of
fine-grained billing and freedom from server management. Provider-hosted
orchestrators can multiplex over many users and amortize the cost, but still
incur the expenses of dedicated hardware resources and on-call engineering
teams.

Furthermore, standalone orchestrators also preclude users from making
application-specific trade-offs and optimizations. While the interface and
implementation of the orchestrator might efficiently support the needs of many
application, it cannot meet all applications' needs, resulting in a compromise
...
performance by using less-efficient but supported patterns or first repeat the
hard work of building and managing their own custom
orchestrator~(\S~\ref{sec:eval:excamera}). Moreover, an application with
...
developers as it gives applications more flexibility to add additional
patterns as needed and apply application-specific optimizations in
orchestration. Moreover, as cloud providers improve the performance of
essential serverless components, application-level orchestration built on top
of those services can automatically become more performant.}
...
scalable and strongly consistent datastores, which is already an essential
building block of serverless applications, resolve the hardest challenge of
