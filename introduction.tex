...
application states (e.g., outstanding invocations and intermediate results)
centrally.}
...
under-utilization and break serverless' benefits of fine-grained billing and
...
\majoredits{In this paper, we argue that additional standalone orchestrators
for serverless applications are unnecessary. Furthermore, decentralized,
application-level orchestration is better for both serverless providers and
serverless users. We argue that application-level orchestration is better for
cloud providers as they need not host and maintain yet another complex
service, saving engineering and hardware resources. It is better for
applications as it gives them more flexibility to implement customized
...
centralization: coordination. Using such datastores, we show that an
application-level library running in-situ with user functions can orchestrate
complex execution graphs efficiently with strong execution guarantees.}
...
executions of non-deterministic functions. And \name{} fan-ins use objects in
