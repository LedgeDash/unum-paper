...
\remove{consisting of} \dhledit{with two critical building blocks:} stateless
...
end-user cost.} \dhledit{Functions are stateless and event-driven. Applications
already require storage services (to persist states).}
...
\dhledit{Serverless building blocks are already highly performant, scalable,
and offers fine-grained pay-for-what-you-use billing. \dhl{Can also remove
this paragraph. It doesn't add much to the paper.}}
...
Cloud providers have deployed specialized \remove{centralized}
\dhledit{standalone} workflow orchestrators~\cite{aws-step-functions,
google-cloud-composer, google-workflows, durable-functions} to support such
applications. \dhledit{Although they are likely internally distributed,
standalone orchestrators work as \emph{logically centralized} controllers.}
...
execution of this graph \dhledit{by invoking functions and receiving function
results} and hosts the state of execution (outstanding invocations and
...
if striked-out text is relevant to our argument. In particular, why amortizing
the costs \emph{causes} ("As a result") orchestrators to offer high-level
programming interfaces and exactly-once guarantees.}
...
\dhledit{However, standalone orchestrators have important drawbacks for both
cloud providers and application developers. Standalone orchestrators require
providers to repeat the costly burden of (building,) hosting and scaling yet
another performance-critical service. Standalone orchestrators also preclude
users from making their own trade-offs between available interactions or
execution guarantees and performance, resource overhead,
\remove{scalability}(\dhl{need example for this}) and
\remove{expressiveness}(\dhl{redundant given "available interactions"?}),
resulting in a compromise familiar from operating
systems~\cite{exokernel,spin}, networks~\cite{active-networks,sdn}, and
storage systems~\cite{comet,splinter}---well designed centralized systems
efficiently support the needs of many applications but cannot meet all
application needs.}

\dhledit{For example, applications that need orchestration patterns not
supported by existing standalone orchestrator services have to either
compromise performance by using less-efficient but supported patterns or first
repeat the hard work of building and deploying their own custom
orchestrator~(\S~\ref{sec:eval:excamera}). An application with deterministic
functions, which requires weaker execution guarantees, may not be able to reap
performance benefits if the orchestrator service only supports strong
...
orchestration optimizations and makes porting applications between different
cloud platforms easier.}

\dhledit{In this paper, we argue that additional standalone services
specialized for serverless orchestration is unnecessary. Furthermore,
decentralized, application-level orchestration is better for both serverless
providers and serverless users. We argue that application-level orchestration
is better for cloud providers as they need not host and maintain yet another
complex service, saving engineering and hardware resources. It is better for
developers as it gives applications more flexibility to apply
application-specific optimizations in orchestration and makes porting
applications between different cloud platforms easier. Moreover, as cloud
providers improve the performance of essential serverless services,
application-level orchestration built on top of those services can
automatically become more performant.}

To support these arguments, we present \name{}, a system for
decentralized\dhledit{, application-level} serverless workflow orchestration
(\S\ref{sec:design}). \name{} provides orchestration as a library that runs
\emph{in-situ} with user-defined FaaS functions, rather than as a
\remove{separate}\dhledit{standalone} service. The library relies on a minimal
...
\dhledit{A key challenge in \name{} is to, in a \emph{decentralized} manner,
support complex stateful orchestration patterns and strong execution
guarantees, without a logically centralized orchestrator. Our insight is that,
scalable and strongly consistent datastores, which already serve as a critical
building block of serverless platforms, resolve the hardest challenge of
centralization: coordination. Using such datastores, we show that a
decentralized orchestrator can achieve strong execution guarantees and run
complex execution graphs efficiently.}

\dhledit{At a high level, \name{} relies on the FaaS scheduler to run each function
...
exactly one output for a function invocation. And \name{} fan-ins use objects
in a consistent datastore as a coordination point for aggregating branches.
Both require generating globally unique names for nodes and edges in the
execution graph \emph{locally} (using only information available at each node
) as well as cleaning up intermediate datastore objects in a timely manner.}
