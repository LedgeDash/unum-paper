...
pywren, gg-atc, beldi, boki}. \shadi{suggestion: maybe name a few apps to give an intuition of the complexity. } These applications call for a \emph{higher-level
...
offering various programming interfaces for workflow definition \shadi{composed of multiple connected functions} (e.g., state
...
duplicate event delivery, never run user code again. \shadi{suggestion: this might be too much detail for here. you can probably remove this paragraph to design rather than intro.}
...
\shadi{I would compress the last 3 paragraphs on the eval. they can each be a single sentence. For example, "Our experimental results show that compared to Step Functions \name{} can improve performance from 11-28\% in case of chaining functions and up to  4.5x in case high-parallelism patterns such as fan-out. At the same time, \name{} significantly reduces the cost from X-Y\%."}

\shadi{have a contributions list, clearly articulating the contributions}
% -------
