...
blocks is challenging. \majoredits{Due to at-least-once execution guarantees
typically provided by FaaS engines, non-deterministic functions may pass
inconsistent results downstream.} Event-driven execution makes depending on
the results of multiple previous functions \majoredits{and therefore fan-in
patterns} challenging. Finally, intermediate states must be managed
...
standalone orchestrators to support such applications
(\S\ref{sec:bg:orchestrator}). Cloud providers commonly offer serverless
...
Centralization makes supporting \majoredits{stateful} interactions
simple---e.g.\ an orchestrator can support fan-in patterns by simply waiting
for all branches to complete before invoking an aggregation function.
Similarly, a centralized orchestrator can ensure that workflow results appear
to be the result of executing each constituent function exactly-once by
choosing one result for each function invocation.
...
\majoredits{For example, applications that need orchestration patterns not
 ...
 orchestrator. \secondedits{A parallel video processing application that
 encodes video chunks in parallel and aggregates results of adjacent branches
 have to compromise performance if the orchestrator only supports aggregating
 the result of all branches (\S\ref
 {sec:eval:excamera}).} Similarly, \secondedits{deterministic applications,
 such as an image resize application or a IoT data processing pipeline for
 aggregating sensor readings, can tolerate duplicate executions without
 compromising correctness, but otherwise cannot reap performance benefits if
 the orchestrator only supports strong exactly-once execution.}}
...
\majoredits{In this paper, we show that additional standalone orchestrators
 ...
 and apply application-specific optimizations in orchestration. It is better
 for cloud providers as it reduces costs by obviating the need to host and
 maintain an additional complex service, which can in turn lower users'
 expenses of running applications}. Moreover, as cloud providers improve the
 performance of essential serverless components, application-level
 orchestration built on top of those services benefit automatically.}
