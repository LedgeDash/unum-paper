...
\majoredits{For example, applications that need orchestration patterns not
 ...
 branches.} Similarly, \secondedits {deterministic applications, such as an
 image resize application or a IoT data processing pipeline for aggregating
 sensor readings, can tolerate duplicate executions without weakening
 correctness, but otherwise cannot reap performance benefits if the
 orchestrator always persists states to ensure exactly-once execution.}}
...
\secondedits{To the best of our knowledge, \name{} is the first system to
 ...
 guarantees on unmodified FaaS infrastructure without requiring additional
 services.}Moreover, while performance and cost are difficult to compare
