...
serverless infrastructure using FaaS schedulers and consistent data stores. In
this section, we evaluate how well application-level orchestration performs,
reduces costs, and improves application flexibility. In particular, we focus
our performance evaluation on whether decentralization comes at a reasonable
overhead compared with logically-centralized standalone orchestrators.}
...
gg-atc}. \secondedits{DynamoDB uses the on-demand provisioning option that
charges per-read and per-write~\cite {dynamodb-pricing}}. To avoid performance
...
\secondedits{All but one application were originally written as Step Function
state machines. For Step Function experiments, we ran them directly with the
...
degrees (as low as 20 branches), Step Functions limits \secondedits{the number
of outstanding fan-out branches}~\cite{aws-step-functions-map-state} while
...
\secondedits{Although it may not be the case that standalone orchestrators
fundamentally have to impose limits on the number of outstanding function
invocations, this example shows that it is at least not trivial to ameliorate
the constraint. On the other hand, as a library, \name{} is free from the need
to design and implement yet-another service that supports parallel
applications well, but can instead provide as much parallelism as FaaS
schedulers and data stores permit. FaaS schedulers and data stores already
support highly-parallel applications well, and \name{}’s performance will
improve automatically when these underlying services further improve.}
...
is \secondedits{fine-grained and often} lower cost. In particular, because
...
\name{} incurs costs directly from those services \secondedits{such as
additional Lambda runtime~\cite{aws-lambda-pricing} and DynamoDB reads and
writes~\cite{dynamodb-pricing}.} 

\secondedits{Furthermore, costs are more fine-grained in \name{} where compute
resources for executing orchestration logic is charged per-millisecond and
storage for persisting states is charged per read and write. On AWS, \name{}'s
fine-grained costs from Lambda and DynamoDB are much cheaper than Step
Functions. For a basic transition in a chaining pattern, Step Functions
charges \$27.9 per 1 million such transitions. On the other hand, \name{}
costs, for 1 million transitions, (1) \$0.42 for $\sim$200ms extra Lambda
runtime to execute orchestration library code, (2) \$1.3942 for 1 DynamoDB
write to checkpoint, (3) \$0.279 for 1 DynamoDB read to check checkpoint
existence, and (4) \$1.3942 for 1 DynamoDB write to garbage collect the
checkpoint. In total, a basic transition in \name{} is about 8x cheaper than
the provider-hosted orchestrator on the same platform (\$27.9 vs
\$3.4874).}

\secondedits{Table~\ref{table:macro} shows the price to run each of
...
consistently cheaper than Step Functions for all applications we tested. }
...
majority\footnote{Writes in DynamoDB cost $\sim$5x more than reads}. This
includes writing checkpoints, writing to coordination set (either for fan-in
for garbage collection) and deleting checkpoints for garbage collection.

Of course, \secondedits{developer-facing pricing is only a proxy for
\emph{actual} costs of hardware and human resources.} However, it is clear
that, in practice, \secondedits{\name{}'s costs are reasonable and, in fact,
often lower than Step Functions}---AWS's native workflow orchestrator. This
suggests that at least applications that currently run on Step Functions could
afford to run using \name{} instead.
...
\begin{figure}[t!]
    ...
    \caption{\secondedits{\name{} ExCamera replicates the application logic
     from gg and mu where the re-encode stage (xcenc) of a branch can start
     immediately when the previous branch completes decoding (xcdec) and my
     own branch completes the initial encoding (vpxenc). Step Functions
     provides a Map pattern~\cite{aws-step-functions-map-state} for parallel
     workloads. However, branches in Map must be identical and Map does not
     support data dependencies between branches. As a result, to ensure
     previous branches' xcdec have completed, all branches must first finish
     and fan-in to Step Functions before starting the xcenc step, essentially
...
\majoredits{Both gg and mu employ standalone orchestrators to proxy
 ...
 mu. However, the Step Functions ExCamera implementation must serialize the
...
flexibility to add new orchestration patterns easily. As a result, the \name{}
...
orchestrator consists of a coordinator server as well as a rendezvous
...
fault-tolerance is achieved by running multiple coordinating instances
