...
(`us-west-1` on AWS and `us-central-1` on Google Cloud). All functions are
configured with 128MB of memory except for ExCamera where we use 3GB of memory
to replicate the setup in the original paper~\cite{excamera, gg}. Both
DynamoDB and Firestore use the serverless option with on-demand provisioning.
...
to a limitation in the AWS State Language. As a result, the more efficient
...
For the Step Functions experiments, we run the workflows in \emph{Standard}
