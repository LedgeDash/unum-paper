...
step in sequence, using the basic \texttt{chain} pattern, and compares with
...
2 storage accesses and one asynchronous invocation to Lambda. Because the
...
increments\footnote{Lambda limits payload size to 256KB for async
invocations.}.

Figure~\ref{single-transition-latency-breakdown} shows the single transition
...
DynamoDB. A faster NoSQL database and Lambda invocation API will automatically
