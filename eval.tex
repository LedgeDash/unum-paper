...
Step Functions definition (\S~\ref{sec:eval:realworld}).
...
instance itself for the orchestration logic. \name{}'s overhead compares
favorably to that of the Step Function's orchestrator for simple, chain,
transitions.}
...
However, in Step Functions, all concurrent fan-out branches must complete
before any fan-in starts. As a result, a Step Functions implementation must
serialize the \texttt{ENCODE} and \texttt{ENCODE-GIVEN-STATE} stages.
