...
writes~\cite{dynamodb-pricing}. Furthermore, costs are more fine-grained in
\name{} where compute resources for executing orchestration logic is charged
per-millisecond and storage for persisting states per read and write.}
...
in practice, \secondedits{\name{}'s costs are reasonable and, in fact, often
lower than Step Functions}---AWS's native workflow orchestrator. This suggests
