...
\begin{enumerate}
	...
	background, readers should know that the workflow definitions
	\emph{logically} expresses a directed graph where nodes are constituent
	functions and edges are transitions. And the orchestrator executes the
	transitions by...}
...
functions.\dhl{again, sync with background. readers should know this is not
hard to do because existing languages logically already express such a
directed graph.} Based on the directed graph, the compiler generates an IR in
...
\dhl{Sync with background. Reader should know by now how serverless workflows
are normally developed. Use AWS Step Functions and Lambda as concrete example.
A serverless workflow with Step Functions and Lambda = A Step Functions
workflow definition written in JSON that expresses a state machine + a set of
lambdas \emph{user code}. When deployed, a SF and a set of lambdas are
created. When running, the SF instance invokes lambdas and lambdas send
results back to SF.}
