...
\begin{enumerate}
	...
	decentralized manner? \shadi{hard to understand the diff between 1 and 2. }
...
delivers better\shadiS{->"comparable or improved"? we are not always better, are we?} performance and cost savings than a state-of-the-art
...
centralized orchestrator. \shadi{the contrast part is not very clear. is it essentially a vs b?} \shadi{if short on space, you can remove b altogether or move it to the side of a) and have them in one row. Then, separate c) into its own figure.  }
...
directed graph representation where nodes are constituent \shadi{better name? "original"? "user-defined"?} functions of the
