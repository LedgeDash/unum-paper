...
running its assigned gadget (Figure~\ref{fig:arch:runtime}). The runtime
...
to aggregate the results. For example, a video encoder might break down a
large video into chunks, encode each in parallel and then concatenant all the
encoded vidoes together. The \texttt{fanIn} gadget takes the outputs from a
vector of functions and invokes a single ``sink'' function. Each function in
the vector is assigned a \texttt{fanIn} egress node and the sink function the
only \texttt{fanIn} ingress node.
